/**
 * Little Math Explorer — Game Engine & State Machine
 * Coordinates session progress, question generation, and score tracking.
 */
...
class GameEngine {
  constructor(options = {}) {
    const defaultSettings = getSEngineHelper ? getSEngineHelper.getSettings() : { soundEnabled: true, maxNumber: 10, questionsPerSession: 10 };
    this.settings = options.settings || defaultSettings;
    const QEngineClass = getQEngineHelper || QuestionEngine;
    this.questionEngine = new QEngineClass({ maxNumber: this.settings.maxNumber });
    this.resetState();
  }

  updateSettings(newSettings) {
    this.settings = { ...this.settings, ...newSettings };
    this.questionEngine.setMaxNumber(this.settings.maxNumber);
  }
  ...
  resetState() {
    ...
    this.state = 'HOME'; // 'HOME', 'PLAYING', 'FEEDBACK', 'COMPLETED'
  }
