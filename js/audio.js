/**
 * Little Math Explorer — Audio Synthesizer Engine
...
class AudioEngine {
  ...
  unlockAudio() {
    try {
      ...
      if (this.audioCtx) {
        ...
        // Play silent 0.01s buffer to unlock iOS AudioContext
  ...
  /**
   * Sound effect: Correct answer (Happy ascending chime C5 -> E5 -> G5)
   */
  playCorrect() {
    if (!this.enabled) return;
  ...
  /**
   * Sound effect: Gentle retry (Soft warm double note)
   */
  playIncorrect() {
    if (!this.enabled) return;
  ...
  /**
   * Sound effect: Game Completion Fanfare (C5 -> E5 -> G5 -> C6 -> E6)
   */
  playComplete() {
    if (!this.enabled) return;
    this.playTone(523.25, 'triangle', 0.15, 0.0, 0.25);
    this.playTone(659.25, 'triangle', 0.15, 0.12, 0.25);
    this.playTone(783.99, 'triangle', 0.15, 0.24, 0.25);
